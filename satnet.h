...
class SatNet{
    ...
    void destroyTree(Sat* node);
    int height(Sat* node);
    int maxValue(int a, int b);
    Sat* findMin(Sat* node);
    Sat* find(Sat* node, int nodeId) const;
    Sat* singleRotateWithLeft(Sat* node);
    Sat* singleRotateWithRight(Sat* node);
    Sat* doubleRotateWithLeft(Sat* node);
    Sat* doubleRotateWithRight(Sat* node);
    Sat* insert(const Sat& satellite, Sat* node);
    Sat* remove(int nodeId, Sat* node);
    int countByInclination(Sat* node, INCLIN degree) const;
    void inOrderList(Sat* node) const;
    Sat* removeDeorbitedHelper(Sat* node);
    Sat* deepCopy(Sat* node);
    int getBalance(Sat* node);
    bool isBalanced(Sat* node);
    Sat* getRoot() const {return m_root;}
    bool traverseAndCheck(Sat* node);
